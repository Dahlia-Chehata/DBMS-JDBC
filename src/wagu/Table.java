...
public final class Table {
	...
	public Table(Board board, int tableWidth, List<String> headersList, List<List<String>> rowsList2) {
		this.board = board;
		if (tableWidth <= 0) {
		...
		for (int i = 0; i < rowsList2.size(); i++) {
			List<String> row = rowsList2.get(i);
			if (row.size() != headersList.size() || row.size() < 0) {
				throw new RuntimeException("Size(" + row.size() + ") of the row(" + i + ") and header size("
						+ headersList.size() + ") are not equal");
			}
		}
		this.rowsList = rowsList2;
		...
		try {
			firstRow = rowsList2.get(0);
			rowHeight = 1;

		} catch (Exception x) {
			firstRow = headersList;
			rowHeight = 0;
